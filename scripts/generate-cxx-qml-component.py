...
def property_declaration(type, name):
    return 'Q_PROPERTY({} {} READ {} WRITE {} NOTIFY {})'.format(
        type, name, property_getter_identifier(name),
        property_setter_identifier(name), property_signal_identifier(name))
...
def function_declaration(ret, identifier, this_qualifier='', args=[]):
    ...
    return '{} {}({}){}{}'.format(ret, identifier, expand_function_args(args),
                                  space_if, this_qualifier)
...
def property_type(s):
    try:
        ...
        raise argparse.ArgumentTypeError(
            'Property must be passed as [type,identifier ...]')
